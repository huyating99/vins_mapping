...
// greater than zero. Return values are in pixels and not scaled.
    std::vector<Eigen::Array2i> RayToPixelMask(const Eigen::Array2i& scaled_begin,
        ...
        ICHECK_GE(scaled_begin.x(), 0);
        ICHECK_GE(scaled_begin.y(), 0);
        ICHECK_GE(scaled_end.y(), 0);
        ...
        if (dy > 0) {
            ...
            ICHECK_NE(sub_y, denominator);
            ICHECK_EQ(current.y(), scaled_end.y() / subpixel_scale);
            return pixel_mask;
        }
